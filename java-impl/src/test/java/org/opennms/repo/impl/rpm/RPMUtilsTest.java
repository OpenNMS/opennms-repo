...
public class RPMUtilsTest {
	...
	public void testGenerateDeltaRPMName() throws Exception {
		...
		deltaRPM = RPMUtils.getDeltaFileName(RPMUtils.getPackage(TestUtils.JRRD1_PATH.toFile()),
				RPMUtils.getPackage(TestUtils.JRRD2_PATH.toFile()));
		assertEquals("jrrd-1.1.0-1_1.1.0-2.el7.centos.x86_64.drpm", deltaRPM);
	}
	...
	public void testCreateDeltaRPM() throws Exception {
		...
		assertEquals(
				TestUtils.JRRD1_PATH.toFile().getParentFile().toPath().normalize().toAbsolutePath().resolve("drpms"),
