...
my $source_ro = OpenNMS::Release::SourceRepo->new("$t/packages/source");
isa_ok($source_ro, 'OpenNMS::Release::SourceRepo');
...
isa_ok($package, 'OpenNMS::Release::SourcePackage');
...
$package = $b->find_newest_package_by_name('test', 'source');
is($package->name, 'test');
...
$package = $a->find_newest_package_by_name('test-package-with-multiple-name-sections', 'source');
...
$b->install_package(OpenNMS::Release::SourcePackage->new("$t/packages/source/test-1.0.tgz"));
...
ok(! -e "$t/testpackages/new/test-1.0.tgz");
my $temp = $b->begin();
$temp->install_package(OpenNMS::Release::SourcePackage->new("$t/packages/source/test-1.0.tgz"));
$temp->commit();
...
sub reset_repos {
	...
	$a = OpenNMS::Release::SourceRepo->new("$t/packages/source")->copy("$t/testpackages/source");
	mkpath("$t/testpackages/new");
	$b = OpenNMS::Release::SourceRepo->new("$t/testpackages/new");
}
